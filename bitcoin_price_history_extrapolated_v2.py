...
def find_closest_date(date_list, target_date):
    # Find the index of the closest date in date_list to target_date
...
def extrapolate_bitcoin_price(dates_historical, prices_historical, end_year=2100):
    ...
    while halving_dates[-1].year < end_year:
        next_halving = halving_dates[-1] + timedelta(days=4*365)
        halving_dates.append(next_halving)
    
    cycles = []
    for i in range(len(halving_dates) - 1):
        # Find the closest dates to halving events
        start_index = find_closest_date(dates_historical, halving_dates[i])
        end_index = find_closest_date(dates_historical, halving_dates[i+1]) if i+1 < len(halving_dates) else None
        
        if end_index is not None and end_index != -1:
            start_price = prices_historical[start_index]
            end_price = prices_historical[end_index]
            growth_rate = (end_price / start_price) ** (1 / 4) - 1  # Annual growth rate over 4 years
            cycles.append(growth_rate)
    
    avg_cycle_growth = np.mean(cycles) if cycles else 0.20  # Default to 20% if no data
    
    for year in range(last_date.year + 1, end_year + 1):
        for month in range(1, 13):
            current_date += timedelta(days=30)
            dates_extrapolated.append(current_date)
            
            growth_rate = avg_cycle_growth
            if current_date in halving_dates:
                growth_rate *= 2  # Increase growth rate post-halving
            elif (current_date.year - last_date.year) % 2 == 1:
                growth_rate *= 0.5  # Simulate a correction or lower growth in off-halving years
...
plt.savefig('bitcoin_price_history_extrapolated_v2.png', dpi=300, bbox_inches='tight')
